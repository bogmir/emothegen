...
<xsl:stylesheet 
    ...
    <xsl:template match="tei:TEI">
        <html>
            <head>
                ...
                <xsl:processing-instruction name="php">include_once 'includes_cabecera.php'<xsl:text>?</xsl:text></xsl:processing-instruction>
            </head>
            <body>
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="tei:fileDesc/tei:titleStmt/tei:title[@type='traduccion']">
                        <h2 id="autor">
                            ...
                            <xsl:text>, </xsl:text>
                            <span style="text-transform:uppercase;"><xsl:value-of select="tei:fileDesc/tei:titleStmt/tei:title[@type='original']"/></span>
                        </h2>
                        <h1 id="titPrincipal">
    ...
    <xsl:template match="tei:text" mode="toc">
        ...
        <div id="menu_herramientas">
            ...
            <div id="b_panel">
                <span onClick="mostrarPanel(0)">    <xsl:value-of select="$labels/cerrar_panel[@lang=$pLang]"/>
                </span>
            </div>
            
            <div id="panel">
                <span id="linea_tools"><strong><xsl:value-of select="$labels/indice_navegacion[@lang = $pLang]"/></strong></span>
                <br/>
                <a href="#metadatos"><xsl:value-of select="$labels/datos_edicion[@lang = $pLang]"/></a>
                <br/>
                        ...
                        <xsl:when test="($etiquetaParatexto = 'elenco' or 
                            ...
                            <a href="#{$etiquetaParatexto}">
                                <xsl:choose>
                                    ...
                                    <xsl:otherwise>
                                        <xsl:text>[</xsl:text><xsl:value-of select="$labels/*[local-name() = $etiquetaParatexto][@lang = $pLang]"/><xsl:text>]</xsl:text>
                                    </xsl:otherwise>
                                </xsl:choose>
                                <br/>
                            </a>
                ...
                <xsl:for-each select="tei:body/tei:div1">
                    <xsl:variable name="tipoSeccion"><xsl:value-of select="@type"/></xsl:variable>
                    <xsl:variable name="etiquetaActo">
                        <xsl:value-of select="$tipoSeccion"/><xsl:text>_</xsl:text><xsl:value-of select="@n"/>
                    </xsl:variable>
                ...
                <xsl:for-each select="tei:back/tei:div">
                    <xsl:variable name="etiquetaParatexto"><xsl:value-of select="@type"/></xsl:variable>
                    <xsl:variable name="headerParatexto">
                        <xsl:if test="tei:head and tei:head!=''">
                            <xsl:value-of select="concat(translate(substring(tei:head, 1, 1), $lowercase, $uppercase), substring(tei:head, 2))"/>
                        </xsl:if>
                ...
                <!-- Display features -->
                <hr/>
                <span id="linea_tools"><strong><xsl:value-of select="$labels/marcas_visuales[@lang = $pLang]"/></strong></span>
                
                <!-- Mostrar cada -->
                <p>
                    <label>
                        <xsl:value-of select="$labels/no_lineas[@lang = $pLang]"/><br/>
                        <span id="etiquetaVersos" onClick="Resaltar('numLineas')"><xsl:value-of select="$labels/mostrar_cada[@lang = $pLang]"/></span>
                    </label>
                </p>
                ...
                <p>
                    <label>
                        ...
                        <span id="etiquetaAcotacion"><xsl:value-of select="$labels/acotaciones[@lang = $pLang]"/></span>
                    </label>
                ...
                <xsl:if test="(substring($nombreFichero,1,2)!='C6')">
                    <p>
                        <label>
                            ...
                            <span id="etiquetaAparte"><xsl:value-of select="$labels/apartes[@lang = $pLang]"/></span>
                        </label>
                ...
                <xsl:if test="$extent='linea'">
                    <p><xsl:value-of select="$labels/def_tipo_linea[@lang = $pLang]"/></p>
                </xsl:if>
             
                <!-- METRICA -->
                <xsl:if test="(substring($nombreFichero,1,2)!='C6') and (($idioma='es-ES') or ($idioma='pt-PT'))">
                        <label><input type="checkbox" id="mostrarMetrica" onClick="funcionMetrica()" />
                            <div id="muestra_metrica"><xsl:value-of select="$labels/mostrar_metrica[@lang = $pLang]"/></div>
                        </label>
                </xsl:if>
                ...
                <span id="linea_tools">
                    <strong> 
                        <xsl:element name="a">
                            <xsl:attribute name="id"><xsl:text>boton_herramientas</xsl:text></xsl:attribute>
                            ...
                            <xsl:attribute name="title">
                                ...
                                <xsl:text> - </xsl:text><xsl:value-of select="$labels/estadisticas[@lang = $pLang]"/><!-- - STATISTICS-->
                            </xsl:attribute>
                   ...
                   <span id="linea_tools">
                        <strong> 
                            <xsl:element name="a">
                                ...
                                <xsl:attribute name="title">
                                    <xsl:value-of select="/tei:TEI/tei:teiHeader/tei:fileDesc/tei:titleStmt/tei:title[1]"/>
                                    <xsl:text> - </xsl:text><xsl:value-of select="$labels/etiquetas[@lang = $pLang]"/><!-- - TAGGED FEATURES-->
                                </xsl:attribute>
    ...
    <xsl:template match="tei:front">
        ...
        <div id="front">
            ...
            <p class="posicion_ancla"><a name="metadatos"></a></p>
            <div id="metadatos">
                <!--<xsl:text>Edición digital a partir de </xsl:text>-->
                <xsl:if test="(substring($nombreFichero,1,2)!='C6')">
                    <xsl:variable name="fiabilibad" select="//tei:author/@ana" /><br/>
                    <xsl:if test="$fiabilibad='fiable'">
                        <!--Fiable-->    
                    </xsl:if>
                    <xsl:if test="$fiabilibad='probable' or $fiabilibad='dudosa' or $fiabilibad='inautentica'">
                        <strong><xsl:value-of select="$labels/autoria[@lang = $pLang]"/></strong>
                        <xsl:value-of select="$labels/*[local-name() = $fiabilibad][@lang = $pLang]"/>    
                        <br/> 
                    </xsl:if>
                </xsl:if>
                
                <strong><xsl:value-of select="$labels/texto_utilizado[@lang = $pLang]"/></strong><br/>
            ...
            <div id="investigadores">
                <xsl:if test="(substring($nombreFichero,1,2)!='C6')">
                    <xsl:if test="$editorc60">
                        <strong><xsl:value-of select="concat(translate(substring($label_adaptacion_digital, 1, 1), $lowercase, $uppercase), substring($label_adaptacion_digital, 2))"/></strong><br/>
                    </xsl:if>
                ...
                <xsl:if test="(substring($nombreFichero,1,2)='C6')">
                    <strong><xsl:value-of select="$labels/cargo_edicion[@lang = $pLang]"/></strong><br/>
            ...
            <xsl:for-each select="tei:div">
                ...
                <xsl:choose>
                    <xsl:when test="$etiquetaParatexto = 'title_page'">
                        <xsl:variable name="cabecera"><xsl:value-of select="tei:head"/></xsl:variable>
                    ...
                    <xsl:when test="@type = 'elenco'">
                        <div id="elenco">
                            <xsl:variable name="cabecera"><xsl:value-of select="tei:head"/></xsl:variable>
                            <p class="posicion_ancla"><a name="elenco"></a></p>
                            <p class="cabecera">
                                <!--<xsl:if test="$cabecera=''">
                            ...
                            <p class="intro"><xsl:value-of select="tei:p"/></p>
                            <div class="center">
                                <table>
                                    <xsl:for-each select="tei:castList/tei:castItem">
                                        <xsl:variable name="ana"><xsl:value-of select="@ana"/></xsl:variable>
                                        <xsl:variable name="nombre"><xsl:value-of select="tei:role"/></xsl:variable>
                                        <xsl:variable name="descripcion"><xsl:value-of select="tei:roleDesc"/></xsl:variable>
                                        ...
                                        <xsl:if test="not(contains($ana, 'oculto'))">
                                            <tr>
                                                <td>
                                                    ...
                                                    <xsl:if test="$descripcion!=''">
                                                        <xsl:value-of select="tei:role" /><xsl:text>, </xsl:text><xsl:value-of select="tei:roleDesc" />
                                                    </xsl:if>
                                                </td>
                                            </tr>
                                        </xsl:if>
    ...
    <xsl:template match="tei:back">
        <div id="back">
            <xsl:for-each select="tei:div">
                <xsl:variable name="etiquetaParatextoBack"><xsl:value-of select="@type"/></xsl:variable> 
                <xsl:choose>                    
    ...
    <xsl:template match="tei:body">
        <div id="body">
            <xsl:for-each select="tei:div1">
                <xsl:variable name="tipoSeccion"><xsl:value-of select="@type"/></xsl:variable>
                <div class="div1">
                    <xsl:variable name="etiquetaActo"><xsl:value-of select="$tipoSeccion"/><xsl:text>_</xsl:text><xsl:value-of select="@n"/></xsl:variable>
                    
                    <!-- Enlace acto -->
                    <xsl:if test="$etiquetaActo!=''">
                        <p><a name="{$etiquetaActo}"></a></p>
                    </xsl:if>
                    <xsl:if test="$etiquetaActo=''">
                        <p><a name="texto"></a></p>
                    </xsl:if>
    ...
    <xsl:template match="tei:div1/tei:head">
        <h2 class="tituloActo" ><xsl:value-of select="concat(translate(substring(., 1, 1), $lowercase, $uppercase), substring(., 2))"/></h2>
    </xsl:template>
    <!--....................................................................-->
    ...
    <xsl:template match="tei:div2/tei:head">
        <h3 class="tituloEscena" >
            <span class="numVersoInterno" name="numVerso"><xsl:value-of select="../@n"/></span>
            <xsl:apply-templates />
        </h3>
    </xsl:template>
    <!--....................................................................-->
    ...
    <xsl:template match="tei:lg">
        <xsl:variable name="tipoEstrofa"><xsl:value-of select="@type"/></xsl:variable>
        <xsl:variable name="nombreEstrofa">
            <xsl:value-of select="$metrica/*[local-name() = $tipoEstrofa]"/>
<!--            <xsl:choose>
                <xsl:when test="$tipoEstrofa='cancion'">
                    <xsl:text>Canción</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='cancion_canzone'">
                    <xsl:text>Canción (canzone)</xsl:text>
                </xsl:when>
                <!-\-<xsl:when test="$tipoEstrofa='cancion_estructura_abierta'">
                    <xsl:text>Canción (estructura abierta)</xsl:text>
                </xsl:when>-\->
                <xsl:when test="$tipoEstrofa='copla_arte_mayor'">
                    <xsl:text>Copla (arte mayor)</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='copla_castellana'">
                    <xsl:text>Copla castellana</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='copla_estructura_abierta'">
                    <xsl:text>Copla (estructura abierta)</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='cuarteta'">
                    <xsl:text>Cuarteta</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='cuarteta_asonantada'">
                    <xsl:text>Cuarteta asonantada</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='cuarteto'">
                    <xsl:text>Cuarteto</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='decima'">
                    <xsl:text>Décima</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='endecasilabos_sueltos_tirada'">
                    <xsl:text>Endecasílabos sueltos (tirada)</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='lira'">
                    <xsl:text>Lira</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='octava_real'">
                    <xsl:text>Octava real</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='pareado_hexasilabo'">
                    <xsl:text>Pareado hexasílabo</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='pareados'">
                    <xsl:text>Pareados octosílabos</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='pareados_endecasilabos'">
                    <xsl:text>Pareados endecasílabos</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='quintilla'">
                    <xsl:text>Quintilla</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='redondilla'">
                    <xsl:text>Redondilla</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='romance_heroico'">
                    <xsl:text>Romance heroico (tirada)</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='romance_tirada'">
                    <xsl:text>Romance (tirada)</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='romancillo_o_endecha'">
                    <xsl:text>Romancillo o endecha</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='seguidilla'">
                    <xsl:text>Seguidilla</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='sestina'">
                    <xsl:text>Sestina</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='sextilla_de_pie_quebrado'">
                    <xsl:text>Sextilla de pie quebrado</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='silva_tirada'">
                    <xsl:text>Silva (tirada)</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='soneto'">
                    <xsl:text>Soneto</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='terceto'">
                    <xsl:text>Terceto</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='verso_suelto'">
                    <xsl:text>Verso suelto</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='nona'">
                    <xsl:text>Nona</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='oitava'">
                    <xsl:text>Oitava</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='cantiga'">
                    <xsl:text>Cantiga</xsl:text>
                </xsl:when>
                <xsl:when test="$tipoEstrofa='quadra'">
                    <xsl:text>Quadra</xsl:text>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:text>ErrorMetrica</xsl:text>
                </xsl:otherwise>
            </xsl:choose>
-->        </xsl:variable>
        <xsl:variable name="parteEstrofa"><xsl:value-of select="@part"/></xsl:variable>
        ...
        <xsl:if test="$tipoEstrofa!=''">
            ...
            <div>
                <xsl:attribute name="class">
                    <xsl:text>lg_</xsl:text><xsl:value-of select="$tipoEstrofa"/><xsl:text>_</xsl:text><xsl:value-of select="$particionEstrofa"/>
                </xsl:attribute>
                <xsl:attribute name="name">
                    <xsl:text>lg_</xsl:text><xsl:value-of select="$tipoEstrofa"/><xsl:text>_</xsl:text><xsl:value-of select="$particionEstrofa"/>
                </xsl:attribute>
    ...
    <!--<xsl:template match="//tei:app">-->
        <xsl:variable name="num"><xsl:value-of select="@n"/></xsl:variable>
    ...
    <xsl:template match="tei:app/tei:lem | tei:app/tei:rdg">
        <xsl:text>- </xsl:text>
        <em><xsl:value-of select="replace(@wit,'#','')"/></em>
        <xsl:text> </xsl:text>
        <xsl:apply-templates/>
        <br/>
    </xsl:template>
...
<!--    <xsl:template match="" >
    ...
    <xsl:template match="tei:seg/tei:note | tei:l/tei:note | tei:p/tei:note | tei:stage/tei:note | tei:speaker/tei:note | tei:head/tei:note | //tei:trailer//tei:note" >
        ...
        <xsl:if test="$tipoNota='editor_digital' 
            ...
            or $tipoNota='editor'">
            <xsl:variable name="num"><xsl:value-of select="@n"/></xsl:variable>
            <xsl:variable name="tipo"><xsl:value-of select="@type"/></xsl:variable>
            ...
            <div class="contenidoNota">
                ...
                <div class="cerrarNota" onclick="cerrarNota({$num})">X</div>
                <strong><xsl:value-of select="$cabeceraNota"/></strong><br/>
                <xsl:apply-templates/>
            </div>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="tei:l">
        ...
        <div class="numVerso" name="numVerso"><xsl:value-of select="@n"/></div>
        <xsl:variable name="tipoVerso"><xsl:value-of select="@part"/></xsl:variable>
        <xsl:variable name="sangrado"><xsl:value-of select="@rend"/></xsl:variable>
        <xsl:choose>
    ...
    <xsl:template match="tei:trailer">
        <div class="final" name="final"><xsl:apply-templates /><!--<xsl:value-of select="."/>--></div>
    </xsl:template>
    <!--....................................................................-->
